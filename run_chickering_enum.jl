...
function run_eval(instance_path::String, output_folder::String, timeout=60.0, verbose=true)
    ...
    # try
    G_ = copy(G)
    res = chickering_enumerate(G_, measure, output_folder)  # Run cpdag enumeration
    ...
    if verbose
        @info "Results for 'chickering_enumerate': Min: $mmin ms, Max: $mmax ms, Mean: $mmean ms, Std: $mstd ms"
    end
    # catch err
